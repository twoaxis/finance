                ...
                PrimaryButton(
                  ...
                  onPressed: () async {
                    ...
                    try {
                      if (valueController.text.isEmpty) {
                      ...
                      } else if (double.parse(valueController.text) > widget.receivable["value"]) {
                        showDialog(
                          context: context,
                          builder: (context) {
                            return AlertDialog(
                              title: Text("Error"),
                              content: Text("Value has to be less than or equal to ${widget.receivable["value"]}."),
                              actions: [
                                TextButton(
                                  onPressed: () {
                                    Navigator.pop(context);
                        ...
                        if (double.parse(valueController.text) == widget.receivable["value"]) {

                          await FirebaseFirestore.instance
                              .collection("users")
                              .doc(FirebaseAuth.instance.currentUser?.uid)
                              .update({"receivables": FieldValue.arrayRemove([widget.receivable])});
                        }
                        else {

                          context.read<ReceivablesCubit>().state[widget.index]["value"] -= double.parse(valueController.text);

                          await FirebaseFirestore.instance
                              .collection("users")
                              .doc(FirebaseAuth.instance.currentUser?.uid)
                              .update({"receivables": context.read<ReceivablesCubit>().state});
                        }
