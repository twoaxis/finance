...
export const LoginForm: React.FC<LoginFormProps> = ({ onSuccess }) => {
  ...
  const [form, setForm] = useState<LoginFieldsState>({ email: '', password: '' })
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (hasFailedSubmit) validateField(name as keyof LoginFieldsState, value)
  }
  ...
  const isLockedOut = hasFailedSubmit && (!!errors.email || !!errors.password)

  return (
    <Form onSubmit={onSubmit} noValidate>
      {error && <ErrorMessage message={error} />}

      <LoginFields email={form.email} password={form.password} errors={errors} onChange={handleChange} />
