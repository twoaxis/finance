...
export const Balances = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      await updateDoc(userDocRef!, { balances: newList });
  ...
  const handleDelete = async (index: number) => {
    ...
    const newList = balances.filter((_: any, i: number) => i !== index);
    try {
      await updateDoc(userDocRef!, { balances: newList });
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
              ...
              <CardContent className="flex items-center justify-between p-4">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-on-surface-variant">Current Balance: {formatCurrency(item.value)}</p>
                  </div>
                </div>
      ...
      <SimpleModal
        ...
        title={editingIndex !== null ? "Edit Balance" : "Add Balance"}
      >
