...
export function ForgotPasswordPage() {
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      setModalMessage("Check your email for reset instructions.");
      setModalOpen(true);
    } catch (err: any) {
      setModalTitle("Error");
  ...
  return (
    <div className="min-h-screen flex flex-col items-center p-6 bg-surface-light dark:bg-surface-dark relative">
      ...
      <div className="absolute top-6 left-6 z-20">
        <Link to="/login" className="p-2 -ml-2 inline-block rounded-lg hover:bg-container-light dark:hover:bg-container-dark transition-colors">
      ...
      <div className="w-full max-w-sm mx-auto flex-1 flex flex-col justify-center z-10 py-12">
        <div className="flex flex-col items-center mb-10 text-center">
          ...
          <p className="text-xl text-gray-600 dark:text-gray-400">Enter your email below</p>
        </div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={modalTitle}>
        ...
        <div className="mt-6 flex justify-end">
          <PrimaryButton text="Okay" onClick={() => setModalOpen(false)} />
        </div>
      </Modal>
    </div>
  );
