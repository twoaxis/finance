...
export function BillsPage() {
  const { userData, addBill, removeBill, updateBills, updateBalances, updateBudget } = useUserData();
  ...
  const handleOpenEdit = (item: Bill) => {
    setEditingItem(item);
    setName(item.name);
  ...
  const handleOpenPayment = (item: Bill) => {
    setPaymentItem(item);
    setPaymentAmountStr(item.value.toString());
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingItem) {
        const newItems = (userData?.bills || []).map(i => 
          i.name === editingItem.name ? { name, value: val } : i
        );
        await updateBills(newItems);
      } else {
        await addBill({ name, value: val });
      }
      setModalOpen(false);
    } catch (err) {
      console.error(err);
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // 2. Deduct from Balance (if selected)
  ...
  const total = userData?.bills.reduce((sum, item) => sum + item.value, 0) || 0;
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6 pb-24">
      ...
      <div className="bg-gradient-to-br from-primary to-primary-dark rounded-3xl p-6 text-white shadow-lg mb-8">
        <h2 className="opacity-90 font-medium mb-1">Total Recurring Bills</h2>
        <div className="text-4xl font-bold">{formatMoney(total, userData?.currency)}</div>
      </div>

      <div className="space-y-3">
        {(userData?.bills || []).map((item) => (
          <div
            key={item.name}
      ...
      {/* Edit Modal */}
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editingItem ? "Edit Bill" : "Add Bill"}>
        <div className="space-y-4">
          <InputField label="Name" value={name} onChange={setName} placeholder="Internet, Rent, etc." disabled={pending} />
          <InputField label="Monthly Amount" value={valueStr} onChange={setValueStr} type="number" placeholder="0.00" disabled={pending} />
